  ...
  commit() {
    if(this._geomIsDirty) {
        ...
        for (const attr in this._attributes) {
            ...
            const indexedBuffer = this._attributes[attr].data
            ...
            switch (dimensions) {
                case 3:
                    geom.addAttribute(attr, new THREE.BufferAttribute(indexedBuffer, dimensions))
                    break;
                case 2:
                    geom.addAttribute(attr, new THREE.BufferAttribute(indexedBuffer, dimensions))
                    break;
        ...
        if (!this._geomId) {
            ...
            this._fragId = this._interface.modelBuilder.addFragment(this._geomId, 'default')

            this._interface.modelBuilder.changeFragmentGeometry(this._fragId, this._geom)
        } else {
            this._interface.modelBuilder.changeGeometry(this._geom, this._interface.modelBuilder.packNormals(geom));
            this._geom = geom;
        }
    }
...
class HydraMaterial {
  ...
  assignTexture(mainMaterial, parameterName, materialParameterMapName, materialParameterName, parameterScaleValue) {
  ...
  updateFinished(type, relationships) {
    ...
    this.assignTexture(mainMaterialNode, 'clearcoat', 'clearcoatMap', 'clearcoat', 1.0);
    this.assignTexture(mainMaterialNode, 'clearcoatRoughness', 'clearcoatRoughnessMap', 'clearcoatRoughness', 5.0);
    this.assignTexture(mainMaterialNode, 'roughness', 'roughnessMap', 'roughness', 1.5);
    this.assignTexture(mainMaterialNode, 'metallic', 'metalnessMap', 'metalness', 1.0);
